...
export default function VideoCall() {
  ...
  const reveal = () => {
    ...
    timer.current = setTimeout(() => setShow(false), HIDE_DELAY)
  }
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e) => {
      if (window.innerHeight - e.clientY <= PROXIMITY) reveal()
    }
    const onTouchOrClick = () => reveal()

    el.addEventListener('mousemove', onMouseMove)
    el.addEventListener('touchstart', onTouchOrClick)
    el.addEventListener('click', onTouchOrClick)

    // Initial reveal, then hide after delay
    reveal()

    return () => {
      clearTimeout(timer.current)
      el.removeEventListener('mousemove', onMouseMove)
      el.removeEventListener('touchstart', onTouchOrClick)
      el.removeEventListener('click', onTouchOrClick)
    }
  }, [])
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 z-50 bg-black">
      ...
      {/* Local video */}
      <div className="absolute bottom-24 right-4 z-10 h-48 w-36 overflow-hidden rounded-xl border-2 border-gray-700 bg-gray-800 shadow-lg md:bottom-28">
        {localStream && (
          <video
            ref={localVideoRef}
            autoPlay
